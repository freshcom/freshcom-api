defmodule BlueJet.Plugs.Authentication do
  ...
  def verify_access_token(access_token) do
    with {true, %{ "prn" => prn, "exp" => exp } = fields} <- BlueJet.Jwt.verify_token(access_token),
  ...
  def extract_authorization_scope(%{ "prn" => user_id, "aud" => account_id }) do
    {:ok, %{ account_id: account_id, user_id: user_id }}
  end
  def extract_authorization_scope(%{ "prn" => account_id }) do
