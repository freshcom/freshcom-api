defmodule BlueJet.DataTrading.DataImport do
  ...
  defp import_resource(row, account, "Product") do
    ...
    pcm =
      CatalogueService.get_product_collection_membership(
        %{collection_id: collection_id, product_id: product.id},
        %{account: account}
      )
  ...
  defp extract_product_goods_id(%{"goods_code" => code, "goods_type" => "Unlockable"}, account)
       when byte_size(code) > 0 do
  ...
  defp extract_product_collection_id(row, account, rel_name) do
    ...
    cond do
      ...
      row[code_key] && row[code_key] != "" ->
        product_collection =
          CatalogueService.get_product_collection(%{code: row[code_key]}, %{account: account})
