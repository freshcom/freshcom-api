defmodule BlueJet.Balance.Payment.Proxy do
  ...
  def get_owner(payment = %{ owner_type: "Customer" }) do
    ...
    owner = Map.get(payment, :owner) || CrmService.get_customer(%{ id: payment.owner_id }, %{ account: account })
  ...
  def create_stripe_customer(owner, owner_type) do
    ...
    StripeClient.post("/customers", %{ owner_email: owner.email, metadata: %{ owner_id: owner.id, owner_type: owner_type, owner_name: owner.name } }, mode: account.mode)
  end
