defmodule BlueJet.Fulfillment.Policy do
  ...
  #
  def authorize(request = %{role: role, account: account, user: user}, "list_fulfillment_package")
    ...
    authorized_args = %{
      authorized_args
      | filter: filter,
        all_count_filter: %{customer_id: customer.id}
    }
  ...
  def authorize(request = %{role: role}, "list_fulfillment_package")
      when role in ["support_specialist"] do
    authorized_args = from_access_request(request, :list)

    if authorized_args[:filter][:order_id] || authorized_args[:filter][:customer_id] do
      authorized_args = %{
        authorized_args
        | all_count_filter: Map.take(authorized_args[:filter], [:order_id, :customer_id])
      }
  ...
  def authorize(request = %{role: role}, "get_fulfillment_package")
      when role in [
             "customer",
             "support_specialist",
             "read_only",
  ...
  def authorize(request = %{role: role}, "delete_fulfillment_package")
      when role in ["distribution_specialist", "developer", "administrator"] do
  ...
  #
  def authorize(request = %{role: role}, "list_fulfillment_item")
      when role in ["customer", "support_specialist"] do
    authorized_args = from_access_request(request, :list)
  ...
  def authorize(request = %{role: role}, "create_fulfillment_item")
      when role in ["distribution_specialist", "developer", "administrator"] do
    {:ok, from_access_request(request, :create)}
  end

  def authorize(request = %{role: role}, "update_fulfillment_item")
      when role in ["distribution_specialist", "developer", "administrator"] do
  ...
  # MARK: Return Package
  #
  def authorize(request = %{role: role, account: account, user: user}, "list_return_package")
      when role in ["customer"] do
    authorized_args = from_access_request(request, :list)
  ...
  # MARK: Return Item
  #
  def authorize(request = %{role: role}, "create_return_item")
      when role in ["distribution_specialist", "developer", "administrator"] do
    {:ok, from_access_request(request, :create)}
  end
  ...
  # MARK: Unlock
  #
  def authorize(request = %{role: role, account: account, user: user}, "list_unlock")
      when role in ["customer"] do
    authorized_args = from_access_request(request, :list)
  ...
  def authorize(request = %{role: role}, "list_unlock") when role in ["support_specialist"] do
    ...
    if authorized_args[:filter][:customer_id] do
      authorized_args = %{
        authorized_args
        | all_count_filter: Map.take(authorized_args[:filter], [:customer_id])
      }
  ...
  def authorize(request = %{role: role}, "list_unlock")
      when role in ["distribution_specialist", "developer", "administrator"] do
    {:ok, from_access_request(request, :list)}
  end

  def authorize(request = %{role: role}, "create_unlock")
      when role in ["support_specialist", "distribution_specialist", "developer", "administrator"] do
    {:ok, from_access_request(request, :create)}
  end
  ...
  def authorize(request = %{role: role}, "delete_unlock")
      when role in ["support_specialist", "distribution_specialist", "developer", "administrator"] do
    {:ok, from_access_request(request, :delete)}
  end
