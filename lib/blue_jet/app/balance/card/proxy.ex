defmodule BlueJet.Balance.Card.Proxy do
  ...
  def sync_to_stripe_card(card = %{stripe_card_id: nil, source: source})
  ...
  def update_stripe_card(
        card = %{stripe_card_id: stripe_card_id, stripe_customer_id: stripe_customer_id},
        fields
      ) do
    account = get_account(card)

    StripeClient.post(
      "/customers/#{stripe_customer_id}/sources/#{stripe_card_id}",
      fields,
      mode: account.mode
    )
  end
  ...
  def create_stripe_card(
        card = %{source: source, stripe_customer_id: stripe_customer_id},
        metadata
      )
    ...
    response =
      StripeClient.post(
        "/customers/#{stripe_customer_id}/sources",
