defmodule BlueJet.Goods.DefaultService do
  ...
  def delete_all_stockable(opts = %{ account: account = %{ mode: "test" }})  do
    batch_size = opts[:batch_size] || 1000

    stockable_ids =
      Stockable.Query.default()
      |> Stockable.Query.for_account(account.id)
      |> Stockable.Query.paginate(size: batch_size, number: 1)
      |> Stockable.Query.id_only()
      |> Repo.all()

    Stockable.Query.default()
    |> Stockable.Query.filter_by(%{ id: stockable_ids })
    |> Repo.delete_all()

    if length(stockable_ids) === batch_size do
      delete_all_stockable(opts)
    else
      :ok
    end
  end
  ...
  def delete_all_unlockable(opts = %{ account: account = %{ mode: "test" }})  do
    ...
    unlockable_ids =
      Unlockable.Query.default()
      |> Unlockable.Query.for_account(account.id)
      |> Unlockable.Query.paginate(size: batch_size, number: 1)
    ...
    if length(unlockable_ids) === batch_size do
  ...
  def delete_all_depositable(opts = %{ account: account = %{ mode: "test" }})  do
    batch_size = opts[:batch_size] || 1000

    depositable_ids =
      Depositable.Query.default()
      |> Depositable.Query.for_account(account.id)
      |> Depositable.Query.paginate(size: batch_size, number: 1)
      |> Depositable.Query.id_only()
      |> Repo.all()

    Depositable.Query.default()
    |> Depositable.Query.filter_by(%{ id: depositable_ids })
    |> Repo.delete_all()

    if length(depositable_ids) === batch_size do
      delete_all_depositable(opts)
    else
      :ok
    end
  end
