defmodule BlueJet.FileStorage.Policy do
  ...
  def authorize(request = %{role: role}, "list_file")
  ...
  def authorize(%{role: role}, "delete_file") when role in ["anonymous", "guest"],
    do: {:error, :access_denied}

  def authorize(request = %{role: role}, "delete_file")
      when role in ["developer", "administrator"] do
    {:ok, from_access_request(request, :delete)}
  end

  def authorize(request = %{account: account, user: user, role: role}, "delete_file")
  ...
  def authorize(request = %{role: role}, "list_file_collection")
  ...
  def authorize(%{role: role}, "create_file_collection")
  ...
  def authorize(request = %{role: role}, "get_file_collection")
      when role in ["guest", "customer"] do
    authorized_args = from_access_request(request, :get)
  ...
  def authorize(%{role: role}, "update_file_collection")
      when role in ["anonymous", "guest", "customer"],
      do: {:error, :access_denied}
  ...
  def authorize(%{role: role}, "delete_file_collection")
      when role in ["anonymous", "guest", "customer"],
      do: {:error, :access_denied}
  ...
  #
  def authorize(%{role: role}, "create_file_collection_membership")
      when role in ["anonymous", "guest", "customer"],
      do: {:error, :access_denied}

  def authorize(request = %{role: role}, "create_file_collection_membership")
      when not is_nil(role) do
    authorized_args = from_access_request(request, :create)

    fields =
      Map.merge(authorized_args[:fields], Map.take(request.params, ["collection_id", "file_id"]))
  ...
  def authorize(%{role: role}, "update_file_collection_membership")
      when role in ["anonymous", "guest", "customer"],
      do: {:error, :access_denied}

  def authorize(request = %{role: role}, "update_file_collection_membership")
      when not is_nil(role) do
    {:ok, from_access_request(request, :update)}
  end

  def authorize(%{role: role}, "delete_file_collection_membership")
      when role in ["anonymous", "guest", "customer"],
      do: {:error, :access_denied}

  def authorize(request = %{role: role}, "delete_file_collection_membership")
      when not is_nil(role) do
    {:ok, from_access_request(request, :delete)}
  end
